...
const isHeaderComponent = (componentId, HeaderOrderableComponents) => Object.values(HeaderOrderableComponents)
    .some(component => {
        const schemaName = getComponentSchemaName(component);
        const joinedSchemaName = schemaName.replace(/ /g, '');
        return joinedSchemaName.includes(componentId.split(':')[0]);
    });
...
export const PageOrderableContainer = ({ orderableComponents, selectedComponentId,
    replaceIndex }) => {
    const { PageOrderableComponents, HeaderOrderableComponents } = getOrderableComponents();

    const orderableContainer = useRef(); // DOM nodes for PageOrderableContainer div
    ...
    /**
     * Return the array index of the orderable component in PageOrderableContainer DOM nodes
     ...
     * @param {array} containerNodes the orderable component in PageOrderableContainer DOM nodes
    ...
    const onComponentDragOver = () => {
        ...
        if (componentNewPosition && selectedComponent
                && !isHeaderComponent(selectedComponentId, HeaderOrderableComponents)) {
    ...
    const getPaddingForNestedOrderableComponents = id => {
        if (isHeaderComponent(id, HeaderOrderableComponents)) {
            return {
                padding: 0
            };
        }
    ...
    const withOrderableComponent = (Component, { id, data, settings, menus = {} }) => (
        !isComponentNull(Component, data) ? (
            <div {...n('item', selectedComponentId === id && replaceIndex !== null && 'selected').props} id={id} key={id} style={getPaddingForNestedOrderableComponents(id)}>
    ...
    const renderOrderableComponents = components => (
        components.map(
            ({ id, name, data, settings, menus = {} }) => (
                withOrderableComponent(PageOrderableComponents[name], { id, data, settings, menus })
            )
        )
    );
...
PageOrderableContainer.schema = () => ({
    name: 'PageOrderableContainer'
});

PageOrderableContainer.propTypes = {
...
PageOrderableContainer.defaultProps = {
    orderableComponents: [],
    selectedComponentId: '',
    replaceIndex: null
};

export default withListener(PageOrderableContainer);
